...
class CatalogGiftController extends Controller
{
    public function index(Request $request,$campaign_id)
    {
        ...
        $query_for_gifts = CampaignProduct::with("product")->whereCampaign($campaign_id)->canApply();

        // if($min_point && $max_point) {
        //     // ポイントで絞り込み
        //     $query_for_gifts = $query_for_gifts->whereHas('product', function($query){
        //         $query->whereNull('variation_parent_id');
        //     })->whereBetween('point', [$min_point, $max_point]);
        //     if($min_point == "1") {
        ...
        //         $query->whereNull('variation_parent_id');
        //     });
        //     $search_cateogries = ProductCategory::whereId($category_id)->first();
        //     $search_category = $search_cateogries->name;
        // }
        // elseif($keyword) {
        //     // キーワードで絞り込み
        //     $query_for_gifts = $query_for_gifts->whereHas('product', function($query) use($keyword){
        //         $query->where('name', 'like', "%$keyword%");
        //     })->whereHas('product.product_category', function($query) use($category_id){
        //         $query->where('id', $category_id)->orWhere('parent_id', $category_id);
        //     })->whereHas('product', function($query){
        //         $query->whereNull('variation_parent_id');
        //     });
        ...
        if($keyword || $category_id) {
            if($category_id && $min_point || $keyword) {
                $query_for_gifts = $query_for_gifts->whereHas('product', function($query) use($keyword){
                    $query->where('name', 'like', "%$keyword%");
                })->whereHas('product.product_category', function($query) use($category_id){
                    $query->where('id', $category_id)->orWhere('parent_id', $category_id);
        ...
        // おすすめ5件
        $recommends = CampaignProduct::with("product")->whereCampaign($campaign_id)->whereHas('product', function($query){
            $query->whereNull('variation_parent_id');
        })->canApply()->take(5)->get();
