...
@section('content')
    ...
    <div class="user_wrap">
        ...
        <p class="blue">必要ポイント数：<span class="used_point">{{ $gift->getPoint() }}</span>pts</p>
        ...
        @if (isset( $gift->product->description_1 ))
            <p class="detail_text">{{ $gift->product->description_1 }}</p>
        @endif
        @if (isset( $gift->product->description_2 ))
            <p class="detail_text">{{ $gift->product->description_2 }}</p>
        @endif
        @if (isset( $gift->product->notice ))
            <p class="detail_text">{{ $gift->product->notice }}</p>
        @endif
    </div>
    @if(Request::routeIs('campaign.catalog.gift.show'))
        <a class="yellow order_confirm_btn">交換する</a>
